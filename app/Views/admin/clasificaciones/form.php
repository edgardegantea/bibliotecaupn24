...
<h1><?= isset($clasificacion) ? 'Editar Clasificación' : 'Crear Clasificación' ?></h1>
 ...
 value="<?= old('nombre', isset($clasificacion) ? $clasificacion['nombre'] : '') ?>" required>
            </div>
            <button type="submit" class="btn btn-primary"><?= isset($clasificacion) ? 'Actualizar' : 'Crear' ?></button>
        </form>
